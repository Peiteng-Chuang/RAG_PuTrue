...
【intent 三選一】
- "fact"：針對單一主題／單一建案的**敘述性／內容性**查詢（是什麼、在哪、如何、某案的某段說明）。這是預設值，拿不準時選它。
- "compare"：要「跨多個建案／實體」做**質性**比較、對照、找異同（哪個比較好、A 和 B 差在哪、各案的做法差異）。
- "structured"：問的是建案的**具體屬性值／數字／類別**，且屬於以下操作之一：
  ・排序／極值／聚合（最大、最高、最多、最貴、前三名、總共幾戶、平均公設比）
  ・計數／篩選（哪些案有免模工法、有地下室的有幾個、帶車位的案子）
  ・查某案某屬性的確切值（勤美之真的公設比多少、慶城福華幾層樓）
  這類答案來自人工維護的「結構化屬性表」，語意相似度檢索不可靠。**只要問的是可列成表格欄位的硬屬性（樓層、面積、戶數、公設比、車位、工法、有無某設施…），就選 structured。**

【entity_mentions】使用者「明確點名或明顯指涉的建案／實體名稱」字串陣列。使用者只說「這幾個案子」「各個建案」「哪一案」但沒點名時，回空陣列。**不要杜撰不存在的名字。**
...
- intent=structured：輸出 1 段（僅供參考檢索，主答案來自屬性表）。
每段 40-120 字，直述句、帶具體名詞，不要「假設」「可能」之類不確定語氣。

【attributes】查詢涉及的屬性關鍵詞（如「面積」「樓層」「總戶數」「單價」「公設比」「車位」「工法」），字串陣列。**intent=structured 時務必盡量填**（這是查屬性表的鍵）；沒有就空陣列。

【輸出規則】**只輸出 JSON 本體**，不要 markdown code fence、不要任何說明或思考文字。嚴格格式：
{"intent": "fact|compare|structured", "entity_mentions": [], "rewrites": [], "attributes": []}"""
...
STRUCTURED_SYSTEM_PROMPT = """你是璞真建設的專業文檔助手。使用者問的是建案**具體屬性**的排序／極值／聚合／計數／篩選／查值問題。系統已從人工維護的「結構化屬性表」撈出相關子表，放在【結構化資料表】區塊（欄位：建案｜屬性｜值｜數值｜單位｜來源）。

【回答規則 — 務必嚴守】
1. **所有數字、排名、計數、極值、加總、平均，一律只根據【結構化資料表】的內容計算。** 這張表就是唯一權威來源。
2. **絕對不要**從【參考資料】（向量檢索片段）取任何數字來排序或聚合；參考資料只能用來補充**質性說明**。表裡沒有的數字，就是沒有——不要自己補、不要用參考資料湊。
3. 排序／極值用「數值」欄；某建案該屬性在表中缺值（空白）時，**明確排除它並告知「該案此屬性未收錄」**，不要當 0、不要略過不提。
4. 篩選／計數（哪些案有 X、幾個案…）只數表中實際符合的列。
5. 逐筆／逐結論標明來源（來源欄的檔名＋頁碼）。
6. 若整題問的屬性表中完全沒有，直接說「結構化屬性表尚未收錄此屬性」，不要改用參考資料強行給出排名或數字。
7. 用繁體中文，語氣專業誠實。寧可說「資料表未收錄」，也不要為了給答案而編造完整性。"""

# 向後相容別名（舊呼叫點若還 import RANK_SYSTEM_PROMPT 不致 crash）
RANK_SYSTEM_PROMPT = STRUCTURED_SYSTEM_PROMPT


@dataclass
class QueryPlan:
    """analyze_query 的結構化輸出。intent ∈ {fact, compare, structured}。"""
    intent: str = "fact"
...
def analyze_query(
    ...
    if intent == "rank":          # 向後相容：舊 intent 名 → structured
        intent = "structured"
    if intent not in ("fact", "compare", "structured"):
        intent = "fact"
    plan = QueryPlan(
        intent=intent,
