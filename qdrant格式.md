...
PIPELINE_VERSION = "2.1.1"               # 2.1.1：project_name=file_key[1]、doc_type fallback=file_key[0]
...
| 欄位 | 型別 | 用途 |
|---|---|---|
| `metadata.source.project_name` | keyword | 篩建案（= `file_key[1]`，即類別下的建案/子資料夾；類別在 `file_key[0]` 給 doc_type）|
...
| 欄位 | 用途 |
|---|---|
| `metadata.source.doc_type` | 文件種類維度，與 `project_name` 正交。決定優先序：**sidecar 逐檔 `doc_type` 覆寫 > 對照表 `mkdata/doc_type_map.json`（`{資料夾: doc_type}`）> 最上層資料夾名 `file_key[0]` > `未分類`**。「資料夾名即類型」：data_root 設在最上層時 `file_key[0]` 即類別，預設直接當 doc_type，對照表只在「資料夾名 ≠ 想要的類型」時翻譯 |
...
| 2.1.1 | 2026-06-25 | 推導語意調整（搭配 data_root 設在最上層、`file_key = 類別/建案/…/檔`）：`project_name` 改取 `file_key[1]`（建案，原為 `[0]`）；`doc_type` fallback 改為 `file_key[0]`（類別資料夾名，原為「未分類」）。需以最上層 data_root 重切 + 重 ingest 才生效 |
